...
Fud AI Plus proxy env vars: `GEMINI_API_KEY` and RevenueCat's V1-compatible app API key as `REVENUECAT_API_KEY` are required. Optional quota overrides are `FUD_AI_PLUS_FOOD_DAILY_LIMIT`, `FUD_AI_PLUS_SPEECH_DAILY_LIMIT`, `FUD_AI_PLUS_COACH_DAILY_LIMIT`, and `FUD_AI_PLUS_GLOBAL_DAILY_LIMIT`; `REVENUECAT_ENTITLEMENT_ID` defaults to `plus`. Defaults are 30 food analyses, 40 speech transcriptions, 50 Coach messages, and 120 total successful Plus calls per day.
...
```
fud-ai/
├── ios/          # SwiftUI iOS app (shipping on App Store, v3.4.0)
├── android/      # Kotlin + Jetpack Compose app (min SDK 26 / Android 8.0, v1.1.0)
