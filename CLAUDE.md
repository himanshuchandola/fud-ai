...
- **Notifications wiring**: the Android Settings screen has a single master "Notifications" toggle (no per-reminder sub-controls — the iOS-side per-reminder UI was not ported). The toggle controls one daily reminder: a **weight-log nudge at 08:00** ("Time to weigh in / Log today's weight to keep your progress chart accurate."). `NotificationService.scheduleStreakReminder` and `scheduleDailySummary` exist in code for parity but are **not wired to any UI** — only `scheduleWeightReminder` actually fires. Wire-up site is `SettingsViewModel.setNotificationsEnabled`: ON + `canPostNotifications()` → schedule, OFF → cancel. `FudAIApp.onCreate` re-arms on every cold start (gated by the same prefs+permission check) because `AlarmManager` drops scheduled alarms across reboots and some app-update paths — without this, a user who enabled Notifications once would silently stop getting nudges after their next reboot. If you add streak/summary scheduling later, wire those re-arms into the same `onCreate` block.
