...
- **`PLAYSTORE.md`** holds Play Console listing copy for Android — App Name (15 langs), Short Description (15 langs, must avoid price/promotion keywords like "free"/"discount" or Play Console blocks listing promotion), Full Description (English; per-locale optional, ~3000 chars × 14 langs deferred), What's New (15-lang `<lang-tag>` block, paste-ready into the release notes field), categorization, contact info, App content declarations. **Full Description has a 4000-char hard cap** (Play Console truncates anything over). Update on every Android version bump (current header: `v1.0.5 / versionCode 6`). Apoorv pastes fields into Play Console by hand — APPSTORE.md ↔ PLAYSTORE.md are intentionally separate since Play has no Apple-Guideline-2.3.10 restriction (Play description CAN mention iOS / cross-platform availability if useful)
